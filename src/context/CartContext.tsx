...
export type CartItem = { // Just the shape of one item in the cart.
  id: string
  name: string
  price: number
  quantity: number
...
type CartContextType = { // a TypeScript contract: “anything reading cart context gets these fields.”
  items: CartItem[]
...
export function CartProvider({ children }: CartProviderProps) {  // the Provider component that wraps the app and provides the cart data to the components that need it. Wraps the app. <App /> is children. 
  const [items, setItems] = useState<CartItem[]>([])

  function addToCart(product: Product) {
    setItems((currentItems) => { // currentItems is the current cart array (just  a parameter name, not the state itself)
      const existing = currentItems.find((item) => item.id === product.id) // find if the product is already in the cart

      if (existing) {
        return currentItems.map((item) => // if the product is already in the cart, update the quantity
          item.id === product.id
      ...
      return [ // if the product is not in the cart, add it to the cart
        ...currentItems,
        {
          id: product.id,
          name: product.name,
  ...
  function updateQuantity(productId: string, quantity: number) {
    if (quantity <= 0) {
      removeFromCart(productId) // if the quantity is 0 or less, remove the product from the cart
      return
    }

    setItems((currentItems) => // update the quantity of the product in the cart
  ...
  const cartTotal = items.reduce( // calculate the total price of the cart
    (total, item) => total + item.price * item.quantity,
    0, // start with 0
  ) // multiply the price of the item by the quantity and add it to the total

  return ( // CartProvider publishes everything to the rest of the app. 
    <CartContext.Provider // The component that supplies cart data to the tree. Anything inside it (children) can read that data with useCart().  Shares the cart sate and functions via value={...}.
      value={{
        items,
        addToCart,
        removeFromCart,
